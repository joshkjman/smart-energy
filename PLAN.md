...
- **Open, unanswered:** (a) should `stg_weather_forecast` assert its variable set is exactly what's expected? The pivot is static conditional aggregation, so a new or renamed upstream variable is silently dropped rather than erroring — the same class of risk as choosing projection over a crawler. (b) The whole suite is **structural**; a semantic test (e.g. demand peaks in an expected evening window) would catch a class of regression none of the current 19 can. ~~(c) Least privilege: the S3 sync ran as `joshuaman`; what's the minimum the ingestion Lambda actually needs?~~ **Resolved 2026-09:** `s3:PutObject` on one prefix and `logs:CreateLogStream`/`PutLogEvents` on one log group, per function. **(a) partly answered:** the variable-set assertion belongs in *ingestion*, which knows what it requested, rather than staging, which would be guessing.
